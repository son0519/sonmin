...
# 🧹 데이터 전처리
try:
    # '지역(1)' 열을 '지역'으로 이름 변경하고 인덱스로 설정
    df = df.rename(columns={'지역(1)': '지역'}).set_index('지역')
    # 합계 행과 불필요한 첫 행 제거
    df = df.drop(['합계']).iloc[1:, :]
    ...
    st.error("데이터프레임에 필요한 열('지역(1)', '합계')이 없습니다. 원본 파일 형식이 올바른지 확인해주세요.")
    st.stop()
except IndexError:
    st.error("열 이름 형식 오류가 발생했습니다. 열 이름이 '연도.월' 형식인지 확인해주세요.")
    st.stop()
