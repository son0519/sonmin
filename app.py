...
# 🧹 데이터 전처리
try:
    # 엑셀 파일의 CSV 변환 시 나타나는 메타정보 제거
    # 파일의 첫 행을 데이터프레임의 열로 사용
    df = pd.read_csv(file_name, encoding='cp949', header=1) 
    
    # '구분(1)' 열을 '지역'으로 이름 변경 (실제 데이터에 맞게)
    df = df.rename(columns={'구분(1)': '지역'})
    
    # 불필요한 행 제거: '합계' 행과 빈 값 행 제거
